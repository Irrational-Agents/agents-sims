...
from irrationalAgents.logger_config import setup_logger
...
def gen_agent_by_name(name):
    ...
    if not os.path.exists(root_dir):
        logger.error("agent %s not exists!", name)
        return None
...
class Agent:
    def __init__(self, basic_info, memory_folder_path=False):
        ...
        if basic_info.get('personality'):
            ...
            self.short_memory.personality_text = generate_personality(
                basic_info['personality_traits'])
    ...
    def move(self, agents_list, curr_time, event):
        ...
        self.short_memory.curr_time = self.short_memory.curr_datetime.strftime(
            '%H:%M')
        self.short_memory.curr_date = self.short_memory.curr_datetime.strftime(
            '%Y-%m-%d')
